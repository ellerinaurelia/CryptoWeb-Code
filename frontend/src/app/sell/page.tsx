    ...
    <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: '#111', color: '#fff', paddingTop: '100px', paddingBottom: '50px' }}>
      ...
      <div style={{ maxWidth: '500px', width: '90%', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <p style={{ color: '#8a8f9e' }}>Jual aset lu dan cairin jadi cuan USD sekarang! 💸</p>
        </div>
        ...
        <div style={{ backgroundColor: '#1e2029', padding: '35px', borderRadius: '24px', border: '1px solid #2b2f3a', boxShadow: '0 10px 40px rgba(0,0,0,0.6)' }}>
          <form onSubmit={handleTrade} style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
            ...
            {/* Dropdown Pilih Koin */}
            <div>
              <label style={{ display: 'block', color: '#8a8f9e', marginBottom: '10px', fontSize: '14px', fontWeight: '500' }}>Aset yang Dijual</label>
              <select 
                value={coin}
            ...
            {/* Input Jumlah Uang */}
            <div>
              <label style={{ display: 'block', color: '#8a8f9e', marginBottom: '10px', fontSize: '14px', fontWeight: '500' }}>Jumlah Jual ({coin})</label>
            ...
            <div style={{ backgroundColor: '#15161c', padding: '20px', borderRadius: '12px', border: '1px solid #2b2f3a' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px', marginBottom: '10px' }}>
                <span style={{ color: '#8a8f9e' }}>Estimasi Harga Jual</span>
                <span style={{ color: '#fff', fontWeight: 'bold' }}>~ Live Market</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px' }}>
                <span style={{ color: '#8a8f9e' }}>Saldo USD yang Diterima</span>
                <span style={{ color: '#e74c3c', fontWeight: 'bold' }}>~ Otomatis Dihitung</span>
              </div>
            </div>

            {/* Tombol Trade Utama (Warna Merah buat Sell) */}
            <button 
              type="submit" 
              style={{ width: '100%', padding: '18px', backgroundColor: '#e74c3c', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer', fontSize: '16px', marginTop: '5px', transition: '0.3s' }}
      ...
      {showToast && (
        <div style={{ position: 'fixed', top: '100px', right: '30px', backgroundColor: '#e74c3c', color: '#fff', padding: '16px 24px', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '15px', boxShadow: '0 10px 30px rgba(0,0,0,0.5)', zIndex: 1000 }}>
          <span style={{ fontSize: '28px' }}>💰</span>
          <div>
            <p style={{ margin: '0 0 4px 0', fontWeight: 'bold', fontSize: '16px' }}>Penjualan Sukses!</p>
            <p style={{ margin: 0, fontSize: '13px', opacity: 0.9 }}>Berhasil menjual {amount} {coin} jadi USD</p>
          </div>
        </div>
      )}
    </section>
